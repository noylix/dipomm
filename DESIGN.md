...
- Home merchandising:
  - Главная работает как витрина из четырех смысловых входов: Новинки, Зеленые ценники, Фрукты и ягоды, Хиты.
  - Верхние карточки ведут в каталог с готовым фильтром, а товарные полки ниже показывают до 8 релевантных товарных карточек каждая.
  - Карточка товара показывает два независимых сигнала доверия, если данные есть: рейтинг товара по отзывам на товар и рейтинг продавца по отзывам о продавце.
  - Товарные карточки должны иметь фото товара или локальный категорийный fallback; технические пиктограммы вроде `layout-grid` не должны быть главным смысловым сигналом для покупателя.
  - Если в полке нет товаров, блок скрывается; главная не должна имитировать наполненность нерелевантными позициями.
