...
from order_statuses import apply_revenue_order_filters
...
def analytics_page(
    ...
    revenue_scope = apply_revenue_order_filters(orders_scope)

    # Общая статистика
    total_orders = orders_scope.count()
    revenue_orders_count = revenue_scope.count()
    total_revenue = revenue_scope.with_entities(func.sum(Order.total_price)).scalar() or 0
    total_platform_fee = revenue_scope.with_entities(func.sum(Order.platform_fee)).scalar() or 0
    avg_order_value = revenue_scope.with_entities(func.avg(Order.total_price)).scalar() or 0
    ...
    top_products_q = apply_revenue_order_filters(
        db.query(
            Product.name,
    ...
    top_sellers_q = apply_revenue_order_filters(
        db.query(
            User.email,
    ...
    return request.app.state.templates.TemplateResponse(
        request, "analytics",
        {
            ...
            "revenue_orders_count": revenue_orders_count,
