...
def admin_page(request: Request, db: Session = Depends(get_db)):
    ...
    stats = {
        ...
        "revenue_total": float(
            apply_revenue_order_filters(db.query(func.coalesce(func.sum(Order.total_price), 0)))
            .scalar()
            or 0
        ),
        "platform_fee_total": float(
            apply_revenue_order_filters(db.query(func.coalesce(func.sum(Order.platform_fee), 0)))
            .scalar()
            or 0
        ),
...
def admin_manage_page(request: Request, db: Session = Depends(get_db)):
    """Единая панель: каталог, пользователи, заказы, финансы."""
    user = get_optional_user(request, db)
    guard = check_role(user, ["admin"])
    if guard:
        return guard

    from routes.finance_admin import admin_finance_context
    ...
    ctx.update(admin_finance_context(db, request, user))
    return request.app.state.templates.TemplateResponse(request, "admin", ctx)
...
def admin_change_order_status(
    ...
    if desired_status == "cancelled":
        request.session["admin_error"] = "Для отмены заказа используйте кнопку «Отменить заказ»."
        return RedirectResponse(url="/admin/manage", status_code=303)
...
def admin_cancel_order(
    ...
    ok, message = cancel_order(db, order, role="admin", reason=cancel_reason)
    if not ok:
        request.session["admin_error"] = message
        return RedirectResponse(url="/admin/manage", status_code=303)

    db.commit()
    return RedirectResponse(url=f"/admin/manage#order-{order_id}", status_code=303)


@router.post("/order/{order_id}/items/{item_id}/exclude")
def admin_exclude_order_item(
    order_id: int,
    item_id: int,
    request: Request,
    ...
    if not item:
        request.session["admin_error"] = "Позиция заказа не найдена."
        return RedirectResponse(url=f"/admin/manage#order-{order_id}", status_code=303)
    ...
    db.commit()
    request.session["admin_success"] = message
    return RedirectResponse(url=f"/admin/manage#order-{order_id}", status_code=303)
...
def admin_moderation_page(request: Request, db: Session = Depends(get_db)):
    ...
    for seller in farmer_applications:
        if not (seller.seller_application_number or "").strip():
            ensure_seller_application_number(seller)
    db.commit()
    return request.app.state.templates.TemplateResponse(
        request, "manager",
        {
...
def admin_update_farmer_application(
    ...
    if target:
        ...
        application_number = ensure_seller_application_number(target)
        ...
        status_label = FARMER_APPLICATION_STATUS_LABELS.get(normalized_status, normalized_status)
        db.add(Notification(
            user_id=target.id,
            type="system",
            subject=f"Заявка {application_number}: {status_label}",
            body=(
                f"Статус заявки {application_number}: {status_label}."
