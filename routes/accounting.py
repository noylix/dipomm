...
from models import Complaint, Message, Order, OrderItem, Product, Transaction, User
...
def confirm_payout(order_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    db.commit()
    request.session["accounting_success"] = "Выплата фермеру по заказу подтверждена."
    return RedirectResponse("/accounting/", status_code=303)
...
def initiate_refund(
    ...
    ok, message, refunded = refund_order_payment(db, order, reason[:500])
    if not ok:
        request.session["accounting_error"] = message
        return RedirectResponse("/accounting/", status_code=303)
    ...
    db.commit()
    if refunded:
        request.session["accounting_success"] = (
            f"Возврат выполнен. Средства возвращены {refund_channel_label(order)}."
        )
    else:
        request.session["accounting_success"] = f"Статус заказа обновлён. {message}"
    return RedirectResponse("/accounting/", status_code=303)
