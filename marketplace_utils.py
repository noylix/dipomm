...
BULK_PRODUCT_CATEGORIES = frozenset(
    {
        "\u041e\u0432\u043e\u0449\u0438",
        "\u0424\u0440\u0443\u043a\u0442\u044b",
        "\u042f\u0433\u043e\u0434\u044b",
...
PIECE_PRODUCT_UNIT = "\u0448\u0442"
WEIGHT_PRODUCT_UNIT = "\u043a\u0433"
LEGACY_PIECE_UNITS = frozenset(
    {
        "\u043b",
        "\u043b\u0438\u0442\u0440",
        "\u0431\u0430\u043d\u043a\u0430",
...
def is_bulk_product_category(category: str | None) -> bool:
    return (category or "").strip() in BULK_PRODUCT_CATEGORIES


def normalize_product_unit(unit: str | None, category: str | None) -> str:
    """Весовые категории — только кг, остальное — только шт."""
    if is_bulk_product_category(category):
        return WEIGHT_PRODUCT_UNIT
    return PIECE_PRODUCT_UNIT
...
def product_unit(product) -> str:
    ...
    if unit in {WEIGHT_PRODUCT_UNIT, PIECE_PRODUCT_UNIT}:
        return normalize_product_unit(unit, category)
    if unit.lower() in LEGACY_PIECE_UNITS or not unit:
        return normalize_product_unit(None, category)
    return normalize_product_unit(unit, category)
...
def product_image_urls(product) -> list[str]:
    """Уникальные URL фото товара для галереи (порядок sort_order)."""
    urls: list[str] = []
    seen: set[str] = set()
    images = getattr(product, "images", None) or []
    ordered = sorted(images, key=lambda row: (getattr(row, "sort_order", 0) or 0, getattr(row, "id", 0) or 0))
    for row in ordered:
        url = (getattr(row, "image_url", None) or "").strip()
        if url and url not in seen:
            seen.add(url)
            urls.append(url)
    primary = (getattr(product, "image_url", None) or "").strip()
    if primary and primary not in seen:
        urls.insert(0, primary)
    elif not urls and primary:
        urls.append(primary)
    return urls
...
def product_on_sale_clause(product_model):
    """Товар со скидкой (зелёный ценник)."""
    return and_(
...
def product_not_on_sale_clause(product_model):
    """Товар без активной скидки (обычный ценник)."""
    return or_(
        product_model.discount_price.is_(None),
        product_model.discount_price <= 0,
        product_model.discount_price >= product_model.price,
    )
...
def effective_product_price_expr(product_model):
    return case(
        (
            product_on_sale_clause(product_model),
