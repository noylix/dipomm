...
REVENUE_EXCLUDED_ORDER_STATUSES = frozenset({"cancelled", "canceled", "refunded"})
...
def order_counts_toward_revenue(status: str | None, payment_status: str | None) -> bool:
    """Заказ учитывается в выручке: оплачен и не отменён / не возвращён."""
    if (payment_status or "pending") != "paid":
        return False
    return normalize_order_status(status) not in REVENUE_EXCLUDED_ORDER_STATUSES


def apply_revenue_order_filters(query):
    """Ограничить SQLAlchemy-запрос заказами, которые входят в выручку."""
    from models import Order

    return query.filter(
        Order.payment_status == "paid",
        Order.status.notin_(tuple(REVENUE_EXCLUDED_ORDER_STATUSES)),
    )
